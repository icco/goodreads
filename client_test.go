...
func TestShelf_reportsTruncation(t *testing.T) {
	...
	if len(books) != DefaultMaxPages {
		t.Errorf("got %d books, want the %d pages read before the cap", len(books), DefaultMaxPages)
	}
}

// MaxPages is configurable so a caller with a huge shelf can raise the cap and
// one that only wants a sample can lower it.
func TestShelf_honorsCustomMaxPages(t *testing.T) {
	id := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		id++
		_, _ = fmt.Fprint(w, feed(feedItem(fmt.Sprint(id), fmt.Sprintf("Book %d", id), "A", "")))
	...
	c := &Client{URL: srv.URL, MaxPages: 3, httpClient: srv.Client()}
	books, err := c.Shelf(context.Background(), "12680", ShelfToRead)
	if !errors.Is(err, ErrTruncated) {
		t.Fatalf("err = %v, want ErrTruncated", err)
	}
...
// A zero MaxPages is the unset zero value, not a request to read nothing.
func TestClientZeroMaxPagesUsesDefault(t *testing.T) {
	t.Parallel()
	if got := (&Client{}).maxPages(); got != DefaultMaxPages {
		t.Errorf("maxPages() = %d, want %d", got, DefaultMaxPages)
	}
	if got := (&Client{MaxPages: -1}).maxPages(); got != DefaultMaxPages {
		t.Errorf("maxPages() with a negative cap = %d, want %d", got, DefaultMaxPages)
	}
}
