...
const (
	...
	// ItemsPerPage is how many books Goodreads puts in one RSS page.
	ItemsPerPage = 100

	// DefaultMaxPages bounds a shelf walk at 4,000 books. The walk normally ends
	// on its own when a page comes back empty; this only catches a feed that
	// stops advancing.
	DefaultMaxPages = 40
)
...
// Client fetches shelves from Goodreads.
type Client struct {
	// URL is the Goodreads base URL; overridable for tests.
	URL string
	// MaxPages bounds how far Shelf walks before giving up and returning
	// ErrTruncated. Zero means DefaultMaxPages.
	MaxPages   int
...
// SetHTTPClient overrides the HTTP client, for callers who want their own
// timeout, transport, or instrumentation.
func (c *Client) SetHTTPClient(h *http.Client) { c.httpClient = h }

// maxPages resolves the configured cap, defaulting when unset.
...
// empty.
//
// userID is the number in the profile URL, goodreads.com/user/show/<id>-name.
// That is a different namespace from the author id in /author/show/<id>:
// passing an author id returns some unrelated person's shelves rather than an
// error, so a mixed-up id fails silently and plausibly.
//
...
func (c *Client) Shelf(ctx context.Context, userID, shelf string) ([]Book, error) {
	...
	if !complete {
		return out, fmt.Errorf("%w: shelf %q exceeds %d pages (%d books read)",
			ErrTruncated, shelf, limit, len(out))
	}
	return out, nil
}
